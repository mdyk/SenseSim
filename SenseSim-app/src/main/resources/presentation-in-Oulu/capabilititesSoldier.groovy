...
rfidMapping.put("urn:epc:id:sgtin:0754741.897745.6581" , "9x19 mm 15 rnds magazine");
...
rfidMapping.put("urn:epc:id:sgtin:3172857.166799.8259" , "7,62x51 mm 50 rnds magazine");
...
def  sendMessageWithUpdate(Soldier soldier, DeviceAPI device) {
    ...
    if (interfaceId != 0) {
        ...
        } else if(neighbours.contains(11)) {
            def message = new JsonBuilder( soldier )
            device.api_sendMessage(System.currentTimeMillis() + (device.api_getMyID() * 100000), device.api_getMyID() , 11, interfaceId , message.toPrettyString() , message.toPrettyString().getBytes().length);
            println("sending message to commander ")
            println(message.toPrettyString())
